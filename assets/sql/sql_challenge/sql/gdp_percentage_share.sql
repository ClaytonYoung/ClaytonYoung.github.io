...
-- for each, summed up where desired continent (or not), divided by sum of all continents gdp, rounded to 4, X by 100, casted to text, and appended "%" to a new column with cont of interest
	CAST(TRUNC(ROUND(sum(CASE WHEN continents.continent_code = 'NA' THEN twelve.gdp_per_capita END)/ (sum(twelve.gdp_per_capita)),4)*100,2) AS TEXT)||'%' AS "North America",
	CAST(TRUNC(ROUND(sum(CASE WHEN continents.continent_code = 'EU' THEN twelve.gdp_per_capita  END)/ (sum(twelve.gdp_per_capita)),4)*100,2)  AS TEXT)||'%' AS "Europe",
	CAST(TRUNC(ROUND(sum(CASE WHEN continents.continent_code != 'NA' AND continents.continent_code != 'EU' 
			THEN twelve.gdp_per_capita  END)/ (sum(twelve.gdp_per_capita)),4)*100,2) AS TEXT)||'%' AS "Rest of World"
		FROM twelve 
